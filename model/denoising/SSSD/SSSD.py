...
from model.denoising.SSSD.s4 import S4
...
class BackboneModel(nn.Module):
    def __init__(
        ...
        for i in range(num_residual_blocks):
            residual_blocks.append(
                residual_block(
                    hidden_dim, num_features=output_dim, dropout=dropout
                )
            )
    ...
    def forward(self, input, t, features, x_future):
        ...
        his_feature = self.feature_encoder(x_hist=features, x_future=x_future)
        
        t = self.time_init(self.step_embedding(t))
        x = x.transpose(-1, -2)
        if his_feature is not None:
            his_feature = his_feature.transpose(-1, -2)
        skips = []
        for layer in self.residual_blocks:
            x, skip = layer(x, t, his_feature)
            skips.append(skip)
