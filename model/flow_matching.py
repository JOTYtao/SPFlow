...
class Flow(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # x0 = rnn_pred + 0.1 * torch.randn_like(rnn_pred)
        x0 = x_0 + 0.1 * torch.randn_like(target)
        x1 = target
        with torch.no_grad():
        ...
        v_pred = self(x_t, t_scalar, x_con, rnn_pred)
    ...
    def test_step(self, batch, batch_idx):
        ...
        guidance_scale = getattr(self.hparams, "guidance_scale", self.guidance_scale_default) 
        use_ode = False               
        solver = getattr(self.hparams, "ode_solver", self.ode_solver)   
        with torch.enable_grad():
            with self.maybe_ema():
                if use_ode:                                                                        
                    y_obs = x1 if guidance_scale > 0 else None                                     
                    obs_mask = torch.ones_like(x1) if guidance_scale > 0 else None                 
                    schedule = tsflow_like_schedule(sigmin=self.matcher_sigma, sigmax=1.0) if guidance_scale > 0 else None  
                    y_hat_norm = self.sample_with_ode(                                             
                    x0=x0,
                    x_con=rnn_pred,
                    steps=num_steps,
                    solver=solver,
                    ...
                    y_obs=y_obs,
                    obs_mask=obs_mask,
                    quantiles=torch.linspace(0.1, 0.9, 9) if guidance_scale > 0 else None,
                    schedule=schedule,
